...
<template>
    <Transition name="slide">
        <aside v-show="app.sidebarOpen"
            class="flex flex-col border-r overflow-hidden shrink-0 sidebar-shell"
            :class="[
                app.isDark ? 'bg-slate-900/45 border-slate-700/40' : 'bg-white/85 border-slate-200/90',
                app.isMobile ? 'fixed inset-y-14 left-0 z-40 w-72 shadow-2xl backdrop-blur-xl' : 'w-40'
            ]"
        >
            <!-- 标题栏 -->
            <div class="h-11 flex items-center px-3 gap-2 border-b shrink-0"
                :class="app.isDark ? 'border-slate-700/40' : 'border-slate-200/90'">
                <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
                <span class="text-sm font-semibold tracking-wide flex-1">Workspace</span>
                <button @click="fileStore.loadTree()" class="p-1.5 rounded-lg hover:bg-slate-700/40 transition-colors" title="刷新">
