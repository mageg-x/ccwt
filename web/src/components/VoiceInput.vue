...
async function fetchVoiceStatus() {
    try {
        ...
        backendReason.value = 'voice.statusFailed'
    }
}
...
async function startRecording() {
    ...
    engineLabel.value = 'Baidu'
    if (!backendAvailable.value) {
        const reasonMap = {
            'voice.disabled': t('voice.backendDisabled'),
            'baidu.token.failed': t('voice.authFailed'),
            'voice.statusFailed': t('voice.statusFailed'),
        }
        error.value = `${t('voice.backendUnavailable')}：${reasonMap[backendReason.value] || backendReason.value || 'Unknown'}`
        return
    }
    ...
    try {
        ...
        error.value = t('voice.micPermissionDenied') + ': ' + (e?.message || e)
        cleanupRecorder()
    }
}
...
async function sendToBackend(blob) {
    ...
    try {
        ...
        error.value = e.response?.data?.error || t('voice.recognitionFailed')
    } finally {
...
<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="emit('close')">
        ...
        <div class="relative w-full max-w-sm rounded-2xl shadow-2xl border p-6"
            ...
            <h3 class="text-lg font-semibold mb-4 text-center">{{ t('voice.title') }}</h3>
            ...
            <p class="text-center text-sm text-slate-400 mb-3">
                {{ recording ? t('voice.recording') : processing ? t('voice.recognizing') : t('voice.startRecording') }}
            </p>
            <p v-if="engineLabel" class="text-center text-xs text-slate-500 mb-2">
                {{ t('voice.engine') }}：{{ engineLabel }}
            </p>
            ...
            <div v-if="processing" class="flex items-center justify-center gap-2 mb-3">
                ...
                <span class="text-sm">{{ t('voice.processing') }}</span>
            </div>
